  ...
  <div class="max-w-2xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-3">
        <h1 class="text-2xl font-bold text-gray-900">АвтоТрекер</h1>
        <router-link to="/settings" class="p-2 hover:bg-gray-100 rounded-lg transition text-gray-400 hover:text-gray-600" aria-label="Настройки">
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1 min-w-0">
            <h3 class="font-semibold text-gray-900 text-lg truncate">{{ car.brand }} {{ car.model }}</h3>
            <p class="text-gray-500 text-sm truncate">
              {{ car.year }} · {{ formatMoney(car.mileage) }} км
              <span v-if="car.licensePlate"> · {{ car.licensePlate }}</span>
            </p>
          </div>
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400 shrink-0" viewBox="0 0 20 20" fill="currentColor">
